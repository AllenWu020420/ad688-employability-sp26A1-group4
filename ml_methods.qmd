...
numeric_features = ["SALARY_MID", "MIN_YEARS_EXPERIENCE"]
categorical_features = ["REMOTE_TYPE_NAME", "NAICS_2022_6_NAME"]
...
fig = px.histogram(
    ...
    barmode="group",
    title="Distribution of Remote Work Types Across Clusters"
)
...
fig = px.histogram(
    ...
    color="IS_AI_ML_ROLE",
    barmode="group",
    title="AI Job Distribution Across Clusters"
)
...
KMeans clustering was applied using multiple features including salary, years of experience, AI role classification, and remote work type to segment job postings into distinct groups.

The results reveal three meaningful clusters in the job market.
Cluster 0 represents lower-salary, lower-experience roles, which are mostly entry-level positions with a moderate presence of AI jobs and a relatively higher proportion of remote opportunities.

Cluster 1 contains mid-level roles with moderate salary and experience. This cluster shows the highest concentration of AI-related jobs, indicating that AI roles are more common in mid-tier positions rather than purely senior roles.

Cluster 2 represents higher-salary, higher-experience positions. These roles are less likely to be remote and are dominated by non-AI jobs, suggesting that traditional senior roles still occupy a large portion of high-paying jobs.

Overall, the clustering results indicate that job segmentation is influenced not only by salary and experience, but also by whether a role is AI-related and its remote work characteristics. AI roles tend to concentrate in specific clusters rather than being evenly distributed across all job types.

This suggests that the job market is structured into distinct tiers, where AI-related opportunities and remote work patterns vary significantly across segments.
